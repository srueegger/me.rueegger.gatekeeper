...
|---|---|
| `firefox.desktop` | ~40 lokalisierte `Name[..]`-Zeilen. Der Parser muss Lokalisierungs-Suffixe erkennen und überspringen, ohne dabei langsam zu werden. `Exec=firefox %u` ist nicht absolut und muss über `PATH` aufgelöst werden. |
...
| `duplicate-keys.desktop` | **behalten**, nach Spec gewinnt der erste Wert und der Eintrag bleibt brauchbar |
